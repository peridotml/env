...
import pandas as pd
import polars as pl
from flytekit import Resources, StructuredDataset, task, workflow, WorkflowFailurePolicy
...
def download_movielens_20m_ratings() -> FlyteDirectory:
    ...
    print(f"[1] downloading movielens 20m")
    name = "ml-20m.zip"
...
@task(cache=True, cache_version="v7", requests=Resources(cpu="4", mem="16Gi"), limits=Resources(cpu="4", mem="16Gi"))
...
@task(requests=Resources(cpu="2", mem="1Gi"), limits=Resources(cpu="2", mem="1.1Gi"))
def break_memory():
    array = []
    while True:
